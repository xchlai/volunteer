...
form.addEventListener("submit", async (event) => {
  ...
  message.hidden = true;
  const activityIds = Array.from(activitySelect.selectedOptions).map((option) =>
    Number(option.value)
  );
  const payload = {
  ...
  if (!payload.employee_id || !payload.name || payload.activity_ids.length === 0) {
  ...
  showMessage("登记成功！如再次提交，会覆盖该工号的所有登记记录。", false);
  form.reset();
});
